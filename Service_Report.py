...
class Service_Report:
    def __init__(self, root):
        DB = DatabaseServiceReport()
        DB.conn()

        self.root = root
        self.root.title("Book Keeping - Service Report")
        ...
        Service_Number = StringVar()
        Service_Type = StringVar()
        Service_Date = StringVar()
        Due_Date = StringVar()
        Service_Description = StringVar()
        INR_Total = StringVar()
        INR_Balance_Due = StringVar()
        Ex_Tax = StringVar()
        Bill_To_Address = StringVar()
        Ship_To_Address = StringVar()
        Service_Comments = StringVar()
        ...
        def clear():
            ...
            print("Service : clear method finished\n")

        def insert():
            print("Service : Insert method called")
            if Service_Number.get() != 0:
                DB.insert(
                    customername.get(), Service_Number.get(), Service_Type.get(), Service_Date.get(), Due_Date.get(),
                    Service_Description.get(), INR_Total.get(), INR_Balance_Due.get(), Ex_Tax.get(),
                    Bill_To_Address.get(), Ship_To_Address.get(), Service_Comments.get(), Service_Reference.get(),
                    Ship_To_Name.get(), Shipment_Date.get(), Notes.get(), Salesperson.get(), Department.get()
                )
                serviceList.delete(0, END)
                serviceList.insert(
                    END, customername.get(), Service_Number.get(), Service_Type.get(), Service_Date.get(), Due_Date.get(),
                    Service_Description.get(), INR_Total.get(), INR_Balance_Due.get(), Ex_Tax.get(),
                    Bill_To_Address.get(), Ship_To_Address.get(), Service_Comments.get(), Service_Reference.get(),
                    Ship_To_Name.get(), Shipment_Date.get(), Notes.get(), Salesperson.get(), Department.get(), 
                )
                showInServiceList() # called showInCustomerList method after inserting the data record to database table
                clear()
                print("Service : Insert method finished\n")
            else:
                messagebox.showerror("Book Keeping", "Enter Supplier ID")

        def showInServiceList():
            print("Service : showInServiceList method called")
            serviceList.delete(0, END)
            for row in DB.show():
                serviceList.insert(END, row, str(""))
            print("Service : showInServiceList method finished\n")
        
        def search():
            print("Service : search method called")
            serviceList.delete(0,END)
            for row in DB.search(Service_Number.get()):
                serviceList.insert(END, row, str(""))

            print("Service : search method finished\n")
        
        def delete():
            print("Service :database Delete method called")
            DB.delete(pd[1])
            clear()
            showInServiceList() 
        ...
        def update():
            print("Service Report : Update method called")
            print("pd[1]", pd[1])
            DB.delete(pd[1])
            DB.insert(
                customername.get(), Service_Number.get(), Service_Type.get(), Service_Date.get(), Due_Date.get(),
            ...
            showInServiceList()
            clear()
            print("Service Report : Update method finished")
        ...
        Label(f2, text="Customer Name: ").grid(row=0, column=0, padx=int(0.5), pady=int(0.5))

        Label(f2, text="Service Number: ").grid(row=1, column=0, padx=int(0.5), pady=int(0.5))
        servicenumber = Entry(f2, font="bold", textvariable=Service_Number)
        servicenumber.grid(row=1, column=1, padx=int(0.5), pady=int(0.5))
        
        Label(f2, text="Service Type: ").grid(row=1, column=2, padx=int(2.5), pady=int(2.5))
        servicetype = Entry(f2, font="bold", textvariable=Service_Type)
        servicetype.grid(row=1, column=3, padx=int(2.5), pady=int(2.5))

        Label(f2, text="Service Date: ").grid(row=2, column=0, padx=int(2.5), pady=int(2.5))
        saledate = Entry(f2, font="bold", textvariable=Service_Date)
        ...
        ttk.Checkbutton(f2, text="Is Quote ").grid(row=3, column=0, padx=int(2.5), pady=int(2.5))

        Label(f2, text="Service Description: ").grid(row=4, column=0, padx=int(2.5), pady=int(2.5))
        servicedescription = Entry(f2, font="bold", textvariable=Service_Description)
        servicedescription.grid(row=4, column=1, padx=int(2.5), pady=int(2.5))
        ...
        ttk.Checkbutton(f2, text="Recurring Service ").grid(row=7, column=0, padx=int(2.5), pady=int(2.5))
        ...
        Label(f2, text="Service Comments: ").grid(row=9, column=0, padx=int(2.5), pady=int(2.5))
        servicecomments = Entry(f2, font="bold", textvariable=Service_Comments)
        servicecomments.grid(row=9, column=1, padx=int(2.5), pady=int(2.5))

        Label(f2, text="Customer Reference: ").grid(row=10, column=0, padx=int(2.5), pady=int(2.5))
        customerreference = Entry(f2, font="bold", textvariable=Service_Reference)
        ...
        def saleRec(event):
            print("Service : customerRec method called")
            global pd 
            searchpd = serviceList.curselection()[0]
            pd = serviceList.get(searchpd)
            customername.delete(0,END)
            customername.insert(END, pd[0])
            servicenumber.delete(0, END)
            servicenumber.insert(END,pd[1])
            servicetype.delete(0, END)
            servicetype.insert(END, pd[2])
            saledate.delete(0, END)
            saledate.insert(END, pd[3])
            duedate.delete(0, END)
            duedate.insert(END, pd[4])
            servicedescription.delete(0, END)
            servicedescription.insert(END, pd[5])
            inrtotal.delete(0, END)
            ...
            servicecomments.delete(0, END)
            servicecomments.insert(END, pd[11])
            customerreference.delete(0, END)
            ...
            print("Service : customerRec method finished\n")
        ...
        serviceList = Listbox(f1, width=60, height=22, font=('arial', 12, 'bold'), yscrollcommand=scroll.set, xscrollcommand=scroll2.set)

        serviceList.bind('<<ListboxSelect>>', saleRec)
        serviceList.grid(row=15, column=0, columnspan=4)
        
        scroll.config(command=serviceList.yview)
        scroll2.config(command=serviceList.xview)
        ...
        buttonShowData = Button(f3, text='Show Data', font=('arial', 12, 'bold'), height=1, width=10, bd=4, command= showInServiceList)
