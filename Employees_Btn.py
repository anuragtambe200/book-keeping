...
from Employee_Entitlement import EmployeeEntitlement
from Employee_Entitlement import DatabaseEmployeeEntitlement


class EmployeesBtn:
    def __init__(self, root):
        self.root = root
        self.root.title("Book Keeping - Employees")
        ...
        def entitlement():
            root = Toplevel()
            app = EmployeeEntitlement(root)
            root.mainloop()

        Button(app, text="Employee", height=1, width=15, padx=16, bd=8, font=15, command=employee).grid(row=0, column=0, pady=10, padx=10)

        Button(app, text="Employee Entitlement", height=1, width=15, padx=16, bd=8, font=15, command=entitlement).grid(row=0, column=1)
